...
public class ProductCrawlService {
	...
	public void crawlProducts(String url) {
        ...
        // ex) 보육사 > 시설 교구  등
		//String url = "https://www.boyuksa.co.kr/product/list.html?cate_no=169";
		Document doc = null;
		try {
			doc = Jsoup.connect(url).get();
		} catch (IOException e) {
			log.error("사이트 크롤링이 불가능 합니다.");
			e.printStackTrace();
		}
		...
		for (Element element : elements) {
			...
			try {
				docProduct = Jsoup.connect(toySiteProductLink).get();
			} catch (IOException e) {
				log.error("사이트 크롤링이 불가능 합니다.");
				e.printStackTrace();
			}
			...
			*/
			...
			//String targetImagePath = "C:/lsh/download/image/"; // 이 자리로 옮김 : 11.3
			...
			try {
				...
				log.error("잘못된 URL 입니다.");
				e.printStackTrace();
			} catch (IOException e) {
				log.error("파일복사에 실패했습니다.");
				e.printStackTrace();
			}
			...
			*/
			...
			// 썸네일 저장
			String thumbURL = docProduct.select("li[class='xans-record-'] img[class='ThumbImage']").get(0).attr("src");
			thumbURL = thumbURL.replaceAll("//", "https://");
			String thumb = thumbURL.substring("https://www.boyuksa.co.kr/web/product/small/202302/".length()+1);
			
			log.info("thumbURL : {}", thumbURL);
			log.info("thumb : {}", thumb);
			
			try {
				in = new URL(thumbURL).openStream();
				Files.copy(in, Paths.get(targetImagePath + thumb), StandardCopyOption.REPLACE_EXISTING);
			} catch (MalformedURLException e) {
				log.error("잘못된 URL 입니다.");
				e.printStackTrace();
			} catch (IOException e) {
			...
			// String thumb = docProduct.select("li[class='xans-record-'] img[class='ThumbImage']").get(0).attr("src");
			// thumb = thumb.substring("//www.boyuksa.co.kr/web/product/small/202302/".length()+1);
			productVO.setProductThumbNail(thumb);
			
			// 교정 : 11.8
			productDetailImageURL = productDetailImageURL.substring(productDetailImageURL.lastIndexOf("/")+1);
